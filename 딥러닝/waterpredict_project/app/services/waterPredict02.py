...
def predict_water_electricity():
    ...
    # 데이터 전처리
    df_concat = df_concat[
        (df_concat["총유입수량"] - df_concat["총유입수량"].mean()).abs()
        < 3 * df_concat["총유입수량"].std()
    ]
    df_concat = df_concat[
        (df_concat["전력량"] - df_concat["전력량"].mean()).abs()
        < 3 * df_concat["전력량"].std()
    ]
    ...
    for _ in range(7):
        ...
        new_sequence = np.append(
            last_sequence[:, 1:, :], pred_scaled.reshape(1, 1, -1), axis=1
        )
    ...
    future_dates = pd.date_range(
        start=df_concat["일자"].iloc[-1] + pd.Timedelta(days=1), periods=7, freq="D"
    )
    future_df = pd.DataFrame(
        {"Date": future_dates, "Predicted_wattage": future_predictions_scaled}
    )

    # DBMS에 저장
    engine = create_engine(
        "mysql+pymysql://root:kirito1013@localhost:3306/test?charset=utf8"
    )
